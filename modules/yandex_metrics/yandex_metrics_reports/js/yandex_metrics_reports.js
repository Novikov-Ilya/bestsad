...
(function ($, Drupal, drupalSettings) {
  $(document).ready(function() {
    ...
    var report_list = drupalSettings.yandex_metrics_reports.reportList;
  ...
  */
  function yandex_metrics_reports_load_data(counter_id, filter, type) {
    ...
    var basePath = drupalSettings.path.basePath;
    // Add ?q= to the url if clean urls are turned off.
    var queryParam = (parseInt(drupalSettings.yandex_metrics_reports.cleanUrls, 10) == 0) ? "?q=" : "";
    var ajaxCallbackPath = basePath + queryParam + drupalSettings.path.pathPrefix + "admin/yandex_metrics_ajax/" + counter_id + "/" + filter + "/" + type;
    var indicatorPath = basePath + drupalSettings.yandex_metrics_reports.modulePath + "/images/progress-indicator.gif";
    $.ajax({
